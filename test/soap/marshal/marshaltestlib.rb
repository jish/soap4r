...
module MarshalTestLib
  ...
  def test_time
    ...
    10.times do
      t = Time.now
      marshal_equal(t, t.usec.to_s) { |t| t.usec }
    end
  end

  def test_time_subclass
    marshal_equal(MyTime.new(10)) { |t| t.usec }
  end
  ...
  def test_singleton
    ...
    assert_raises(TypeError) { marshaltest(ARGF) } unless RUBY_VERSION.to_f >= 1.9 # ARGF is no longer a singleton in Ruby 1.9
